...
{
    private static readonly HashSet<string> TenantNotRequiredPaths = new(
    ...
    private static readonly string[] TenantNotRequiredPrefixes = ["/api/users"];
    ...
    )
    {
        ...
        if (string.IsNullOrEmpty(tenantHeader))
        {
            ...
            await context.Response.WriteAsJsonAsync(
                new { message = "Missing X-Tenant-ID header." }
            );
            return;
        }

        await using var configDb = await configFactory.CreateDbContextAsync();
        var org = await configDb.Organizations.FirstOrDefaultAsync(o =>
            o.Slug == tenantHeader && o.IsActive
        );
        ...
        if (org is null)
        {
            ...
            await context.Response.WriteAsJsonAsync(
                new { message = $"Organization '{tenantHeader}' not found or is inactive." }
            );
            return;
        }
